...
days_before_quarantine=27;      % number of days at the beginning of the process that have a larger R0 value due to less restrictions and social distancing
...
omega=50;           % no one is infected for more than omega days, technical parameter that does not need to be accurate, smaller -> faster speed, 
...
%
% average time of infection is 5.8 + 9.1 = 14.9, std = sqrt(2.6^2 + 19.53) = 5.1274
S=(1-normcdf(0:h:omega, 14.9, 5.1274)')./(1-normcdf(0,14.9, 5.1274));    % trimmed normal life length with average of 14 days until clearance
...
R0 = [ones(1,days_before_quarantine/h)*4.5 ones(1,T/h+1-(days_before_quarantine+horizon-detection_time)/h)*1.2 ones(1,(horizon-detection_time)/h)*1.2];
...
[ActiveCases, ActiveCasesByType, ~, TotalCases, TotalCasesByTypes] = BranchingProcessSimulator(sim_num, T, h, S, H, U, Z_0, mu);
...
set(gca,'FontSize',16)
hold on
plot(dates, newcases, 'r', 'LineWidth', line_wd);
h_median_supperimposed=plot(dates(1):dates(end)+horizon, NewCasesPerDay_median(detection_time:end-1), '-', 'Color', [0, 0, 0, 0.5], 'LineWidth', line_wd);
...
legend([h_median_real, h_median_supperimposed, h_CI], 'Median (Real Process)', 'Prediction of observed new daily cases', '90% conf. interval', 'Location', 'NorthWest')
xtickangle(90)
dateaxis('X',2)
ylabel('\bf{Daily New Cases}')
xlabel('\bf{Date}')
title('Daily new cases. Real process and timeshifted daily cases prediction')
print('./Covid_forecast_newcases', '-dpng', '-r0')
...
set(gca,'FontSize',16)
hold on
plot(dates, cumsum(newcases), 'r', 'LineWidth', line_wd);
h_median_supperimposed=plot(dates(1):dates(end)+horizon, TotalCases_median(detection_time:end-1), '-', 'Color', [0, 0, 0, 0.5], 'LineWidth', line_wd);
...
legend([h_median_real, h_median_supperimposed, h_CI], 'Median (Real Process)', 'Prediction of observed total cases', '90% conf. interval', 'Location', 'NorthWest')
dateaxis('X',2)
xtickangle(90)
ylabel('\bf{Total Cases}')
xlabel('\bf{Date}')
print('./Covid_forecast_total', '-dpng', '-r0')
...
% Expectation = k * theta = 9.63, Variance = k * theta^2 = 3.5707^2 => theta = 3.5707^2 / 9.63 = 1.3240, k = 9.63/1.3240 = 7.2734
% Here we assume we have 2 periods of the epidemic in Bulgaria - before the people started distancing themselves and after.
% Also, keep in mind that we can affect the meas
R0 = [ones(1,days_before_quarantine/h)*4.5 ones(1,T/h+1-(days_before_quarantine+horizon-detection_time)/h)*1.2 ones(1,(horizon-detection_time)/h)*0.8];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
mu=@()(mu_matrix);      % the input format for mu is described in the BranchingProcessSimulator.m
...
xlabel('\bf{Date}')
title('Daily new cases. Real process and timeshifted daily cases prediction')
print('./Covid_forecast_newcases_optimistic', '-dpng', '-r0')
%% total number of cases
TotalCasesPerDay= TotalCases(:, 1:(1/h):T/h);
[TotalCases_mean, TotalCases_lower, TotalCases_upper, TotalCases_median]=confInterval(TotalCasesPerDay, 0.10);
...
ylabel('\bf{Total Cases}')
xlabel('\bf{Date}')
print('./Covid_forecast_total_optimistic', '-dpng', '-r0')
...
% Expectation = k * theta = 9.63, Variance = k * theta^2 = 3.5707^2 => theta = 3.5707^2 / 9.63 = 1.3240, k = 9.63/1.3240 = 7.2734
% Here we assume we have 2 periods of the epidemic in Bulgaria - before the people started distancing themselves and after.
R0 = [ones(1,days_before_quarantine/h)*4.5 ones(1,T/h+1-(days_before_quarantine+horizon-detection_time)/h)*1.2 ones(1,(horizon-detection_time)/h)*1.5];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
mu=@()(mu_matrix);      % the input format for mu is described in the BranchingProcessSimulator.m
...
xlabel('\bf{Date}')
title('Daily new cases. Real process and timeshifted daily cases prediction')
print('./Covid_forecast_newcases_pessimistic', '-dpng', '-r0')
...
ylabel('\bf{Total Cases}')
xlabel('\bf{Date}')
print('./Covid_forecast_total_pessimistic', '-dpng', '-r0')
...
% Expectation = k * theta = 9.63, Variance = k * theta^2 = 3.5707^2 => theta = 3.5707^2 / 9.63 = 1.3240, k = 9.63/1.3240 = 7.2734
% Here we assume we have 2 periods of the epidemic in Bulgaria - before the people started distancing themselves and after.
R0 = [ones(1,days_before_quarantine/h)*4.5 ones(1,T/h+1-(days_before_quarantine+horizon-detection_time)/h)*1.2 ones(1,(horizon-detection_time)/h)*4.5];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
mu=@()(mu_matrix);      % the input format for mu is described in the BranchingProcessSimulator.m
...
xlabel('\bf{Date}')
title('Daily new cases. Real process and timeshifted daily cases prediction')
print('./Covid_forecast_newcases_worstcase', '-dpng', '-r0')
...
ylabel('\bf{Total Cases}')
xlabel('\bf{Date}')
print('./Covid_forecast_total_worstcase', '-dpng', '-r0')
...
set(gca,'FontSize',16)
hold on
h_median_supperimposed=plot(dates(1):dates(end)+horizon, ActiveCasesDaily_median(detection_time:end-1), '-', 'Color', [0, 0, 0, 0.5], 'LineWidth', line_wd);
...
xlabel('\bf{Date}')
print('./Covid_forecast_active', '-dpng', '-r0')
...
print('./Covid_PointProcessDensity', '-dpng', '-r0')
%%%
...
print('./Covid_InfectedPeriodPDF', '-dpng', '-r0')
%%%
