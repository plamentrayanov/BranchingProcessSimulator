...
[dates, newcases_hist, totalcases_hist] = readCSVData('../data.csv', 'France');

% the first sevral cases seem controlled and should be excluded as a beginning of the epidemics
...
R0 = [linspace(12, 12, 7/h) linspace(12, 12, 7/h) linspace(12, 12, 7/h) linspace(12, 7.5, 7/h) ...
      linspace(7.5, 4, 7/h) linspace(4, 2, 7/h) linspace(2, 1, 7/h) linspace(1, 0.6, 7/h)...
      linspace(0.6, 0.5, T/h+1-(8*7+horizon-detection_time)/h) linspace(0.5, 0.5, (horizon-detection_time)/h)];
...
[ActiveCases, ~, ~, TotalCases, ~] = BranchingProcessSimulator(sim_num, T, h, S, H, U, Z_0, mu, 'GetAgeStructure', false, 'approx_limit', 5);
...
%% OPTIMISTIC SCENARIO model of the Point process mu, Optimistic Scenario, R0 changes from 0.5 to 0.3, from now on
R0 = [linspace(12, 12, 7/h) linspace(12, 12, 7/h) linspace(12, 12, 7/h) linspace(12, 7.5, 7/h) ...
      linspace(7.5, 4, 7/h) linspace(4, 2, 7/h) linspace(2, 1, 7/h) linspace(1, 0.6, 7/h)...
      linspace(0.6, 0.5, T/h+1-(8*7+horizon-detection_time)/h) linspace(0.3, 0.3, (horizon-detection_time)/h)];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
...
%% PESSIMISTIC SCENARIO model of the Point process mu, Pessimistic Scenario, R0 changes from 0.5 to 0.8, from now on
R0 = [linspace(12, 12, 7/h) linspace(12, 12, 7/h) linspace(12, 12, 7/h) linspace(12, 7.5, 7/h) ...
      linspace(7.5, 4, 7/h) linspace(4, 2, 7/h) linspace(2, 1, 7/h) linspace(1, 0.6, 7/h)...
      linspace(0.6, 0.5, T/h+1-(8*7+horizon-detection_time)/h) linspace(0.8, 0.8, (horizon-detection_time)/h)];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
