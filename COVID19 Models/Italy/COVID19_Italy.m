...
[dates, newcases, totalcases] = readCSVData('../data.csv', 'Italy');
...
ep_start = find(newcases==0, 1, 'last')+1;
dates=dates(ep_start:end);
newcases=newcases(ep_start:end);
totalcases=totalcases(ep_start:end);
...
R0 = [linspace(11.9, 2.5, days_before_quarantine/h) linspace(2.5, 0.8, 7/h) ones(1,T/h+1-(7+days_before_quarantine+horizon-detection_time)/h)*0.8 ones(1,(horizon-detection_time)/h)*0.8];
...
xlabel('\bf{Date}')
title('Measures do not change scenario')
...
% Here we assume we have 2 periods of the epidemic in Bulgaria - before the people started distancing themselves and after.
% we have a week of adjustment towards the quarantine where R0 transitions from 3.9 to 0.85
R0 = [linspace(11.9, 2.5, days_before_quarantine/h) linspace(2.5, 0.8, 7/h) ones(1,T/h+1-(7+days_before_quarantine+horizon-detection_time)/h)*0.8 ones(1,(horizon-detection_time)/h)*0.6];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
mu=@()(mu_matrix);      % the input format for mu is described in the BranchingProcessSimulator.m
...
ylabel('\bf{Daily New Cases}')
xlabel('\bf{Date}')
title('Measures improve scenario')
...
% Here we assume we have 2 periods of the epidemic in Bulgaria - before the people started distancing themselves and after.
% we have a week of adjustment towards the quarantine where R0 transitions from 3.9 to 0.85
R0 = [linspace(11.9, 2.5, days_before_quarantine/h) linspace(2.5, 0.8, 7/h) ones(1,T/h+1-(7+days_before_quarantine+horizon-detection_time)/h)*0.8 ones(1,(horizon-detection_time)/h)*1.2];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
mu=@()(mu_matrix);      % the input format for mu is described in the BranchingProcessSimulator.m
...
ylabel('\bf{Daily New Cases}')
xlabel('\bf{Date}')
title('Measures soften scenario')
...
%% PESSIMISTIC SCENARIO total number of active cases
ActiveCasesDaily= ActiveCases(:, 1:(1/h):T/h);
[ActiveCasesDaily_mean, ActiveCasesDaily_lower, ActiveCasesDaily_upper, ActiveCasesDaily_median]=confInterval(ActiveCasesDaily, 0.10);
