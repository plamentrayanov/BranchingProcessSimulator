...
R0 = [linspace(10, 8, 7/h) linspace(8, 5.5, 7/h) linspace(5.5, 4, 7/h) linspace(4, 2, 7/h) ...
      linspace(2, 2, 7/h) linspace(2, 1.4, 3*7/h) ...
      linspace(1.3, 1.1, T/h+1-(8*7+horizon-detection_time)/h) linspace(1.1, 1.1, (horizon-detection_time)/h)];
...
R0 = [linspace(10, 8, 7/h) linspace(8, 5.5, 7/h) linspace(5.5, 4, 7/h) linspace(4, 2, 7/h) ...
      linspace(2, 2, 7/h) linspace(2, 1.4, 3*7/h) ...
      linspace(1.3, 1.1, T/h+1-(8*7+horizon-detection_time)/h) linspace(0.9, 0.9, (horizon-detection_time)/h)];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
...
R0 = [linspace(10, 8, 7/h) linspace(8, 5.5, 7/h) linspace(5.5, 4, 7/h) linspace(4, 2, 7/h) ...
      linspace(2, 2, 7/h) linspace(2, 1.4, 3*7/h) ...
      linspace(1.3, 1.1, T/h+1-(8*7+horizon-detection_time)/h) linspace(1.3, 1.3, (horizon-detection_time)/h)];
mu_covid_pdf=gampdf(0:h:omega, 7.2734, 1.3240)';
mu_matrix=zeros(size(mu_covid_pdf,1), 1, T/h+1);
mu_matrix(:,1,:)=R0.*repmat(mu_covid_pdf/(sum(mu_covid_pdf)*h),1, T/h+1);
...
for i=1:100
    ...
    plot(dates(1)-detection_time+1:dates(end)+horizon, Im_matrix_Daily(1:end-1), 'Color', [0.7, 0, 0,0.05], 'LineWidth', 1);
end
xtickangle(90)
x_ticks = xticks;
xticks(x_ticks(1):7:x_ticks(end))
...
xlabel('Days')
title('Immigration Scenario (Hypotetical)')
